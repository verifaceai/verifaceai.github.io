...
let performanceChartInstance = null; // Holds the Chart.js instance
...
function navigateTo(pageId) {
  ...
  if (pageId === 'training' && typeof onEnterTrainingPage === 'function') {
    onEnterTrainingPage();
  } else if (pageId === 'quiz' && typeof resetQuiz === 'function') {
    resetQuiz();
...
/**
 * Initializes and updates Chart.js line graph on Dashboard
 */
function renderPerformanceChart() {
  ...
  // If no attempts exist yet, provide initial baseline demo data points
  if (history.length === 0) {
    history = [
      { percentage: 60, timestamp: 'Attempt 1' },
      { percentage: 75, timestamp: 'Attempt 2' },
      { percentage: 85, timestamp: 'Attempt 3' }
    ];
  }
  ...
  // Destroy previous chart instance before re-rendering to prevent canvas overlapping
  if (performanceChartInstance) {
    performanceChartInstance.destroy();
  }

  // Create smooth gradient fill under the line
  const gradient = ctx.createLinearGradient(0, 0, 0, 300);
  gradient.addColorStop(0, 'rgba(79, 70, 229, 0.25)'); // Indigo glow
  ...
  performanceChartInstance = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        pointHoverRadius: 7,
        fill: true,
        tension: 0.35 // Curved line styling
      }]
    },
    options: {
      responsive: true,
